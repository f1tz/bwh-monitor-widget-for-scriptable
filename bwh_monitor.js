...
try {
  ...
  let content = [`📌 Node: ${node_location_id} 🖥 IP: ${ip_addr}`];
  content.push(`🪫 RAM: ${bytesToSize(usedram)} / ${bytesToSize(totalram)}`);
  content.push(`💿 Disk: ${bytesToSize(useddisk)} / ${bytesToSize(totaldisk)}`);
  content.push(`⏳ Data: ${bytesToSize(used)} / ${bytesToSize(total)}`);
  content.push(`⏱️ Reset on ${formatTime(resetday)} (${getResetDaysLeft(resetday)} days left)`);
